...
interface ExperienceBookingStep2Props {
  guests: number;
...
export default function ExperienceBookingStep2({
  guests,
  setGuests,
  selectedSlot,
  note,
        ...
        <div className="ml-0 md:ml-12 border border-slate-200 rounded-xl p-5 w-full md:w-2/3 flex items-center justify-between">
          <div>
            ...
            {selectedSlot ? (
              <p className="text-xs text-slate-500 mt-0.5">
                Max {selectedSlot.max_attendees} people per slot
              </p>
            ) : (
              <p className="text-xs text-slate-500 mt-0.5">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={() =>
                setGuests((prev) =>
                  Math.min(selectedSlot?.max_attendees || 99, prev + 1),
                )
              }
              disabled={!selectedSlot || guests >= selectedSlot.max_attendees}
              className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center hover:bg-slate-50 disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
            >
              +
            </button>
